...
public class PlayerMoving : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            position += new Vector2(horizontal, vertical) * Time.deltaTime * 5f; // 속도 조정
    ...
    private void SendPositionToServer()
    {
        try
        {
            string message = $"{playerId}:{position.x},{position.y}";
            writer.WriteLine(message); // 메시지 전송
        }
        catch (Exception ex)
        {
            Debug.LogError($"좌표 전송 실패: {ex.Message}");
        }
    }
